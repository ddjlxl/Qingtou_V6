...
- [ ] 网络异常时返回统一错误格式
- [ ] 导出 `http` 实例，支持 `http.get<T>()` / `http.post<T>()` 泛型调用
...
**验收标准**：
- [ ] `formatDate('2026-05-04', 'yyyy-MM-dd')` → `'2026-05-04'`
- [ ] `formatDate('2026-05-04T10:30:00', 'HH:mm')` → `'10:30'`
- [ ] `formatMoney(1234.5)` → `'1,234.50'`
- [ ] `isPhone('13800138000')` → `true`
- [ ] `isPhone('123')` → `false`
- [ ] `isRequired('')` → `false`
- [ ] `isRequired('hello')` → `true`
- [ ] `hasRole('admin')` 从 localStorage 读取角色并判断
- [ ] `isAdmin()` 等价于 `hasRole('admin')`
...
**验收标准**：
- [ ] EmptyState 默认显示"暂无数据"
- [ ] EmptyState 支持自定义 icon/title/description
- [ ] LoadingSpinner 默认局部加载动画
- [ ] LoadingSpinner `fullscreen` 模式覆盖全屏
- [ ] 两个组件样式均为 `scoped`
...
**验收标准**：
- [ ] logger 支持 DEBUG/INFO/WARNING/ERROR 四个级别
- [ ] logger 同时输出到控制台和 `logs/app.log` 文件
- [ ] `AppException(code=400, message="参数错误")` 可正常抛出
- [ ] FastAPI 全局异常处理器捕获 AppException 并返回 JSON
...
**验收标准**：
- [ ] `import { http } from '@/shared'` 可用
- [ ] `import { formatDate, isPhone, hasRole } from '@/shared'` 可用
- [ ] `import { EmptyState, LoadingSpinner } from '@/shared'` 可用
- [ ] 后端 `from app.core import logger, AppException` 可用
- [ ] `pnpm type-check` 通过
- [ ] `pnpm lint` 通过
