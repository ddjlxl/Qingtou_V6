...
### 阶段 0 验证
- [ ] Task-01 验证标准全部通过
- [ ] 数据库迁移成功：`alembic upgrade head` 无报错

### 阶段 1 验证
- [ ] Task-02 验证标准全部通过
- [ ] Task-03 验证标准全部通过
- [ ] 端到端验证：调度员创建任务选择"重箱" → 保存成功 → 编辑任务切换为"空箱" → 保存成功 → 运输流水列表显示空重箱状态列

### 阶段 2 验证
- [ ] Task-04 验证标准全部通过
- [ ] 端到端验证：调度员对有司机/车辆的任务点击"标记完成" → 任务状态变为已完成 → 运输流水自动出现对应记录（含空重箱状态）

### 阶段 3 验证
- [ ] Task-05 验证标准全部通过
- [ ] 端到端验证：调度员删除一个已完成的任务 → 运输流水中的对应记录同步消失

### 阶段 4 验证
- [ ] Task-06 验证标准全部通过
- [ ] 端到端验证：创建司机 → 用该司机手机号登录成功 → 修改手机号 → 用新手机号登录成功 → 停用司机 → 登录失败

### 阶段 5 验证
- [ ] Task-07 验证标准全部通过
- [ ] 端到端验证：下载8列模板 → 填写含空重箱状态的数据 → 导入成功 → 运输流水显示空重箱状态

### 阶段 6 验证
- [ ] Task-08 验证标准全部通过
- [ ] Task-09 验证标准全部通过
- [ ] 端到端验证：司机登录 → 看到任务列表 → 点击"开始运输" → 状态变为运输中 → 点击"标记完成" → 状态变为已完成 → 运输流水自动出现记录
