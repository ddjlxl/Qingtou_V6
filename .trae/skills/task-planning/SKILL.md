---
name: task-planning
description: 将技术方案拆解为可执行的开发任务清单，采用垂直切片策略，每个任务适配 TDD 流程。当用户说"拆任务"、"开始规划任务"、"技术方案定了，接下来怎么开发"、"帮我拆一下开发计划"等意图时触发。
version: "2.0.0"
tags: [planning, tasks, development, tdd, vertical-slice]
---
...
开始拆任务前，确认：
1. **技术方案就绪**：`specs/features/<feature-name>/design.md` 存在且包含完整的 AC 覆盖总表
2. **需求文档可查**：`specs/features/<feature-name>/requirements.md` 用于交叉验证 AC 覆盖
...
大多数情况下，直接给出你的判断。但遇到以下情况，停下来确认：
- **切片划分拿不准时**：哪些 AC 归到同一个切片？边界模糊时让用户决定
- **任务粒度拿不准时**：太粗（一个任务做半天）还是太细（一个任务 5 分钟）？
- **执行顺序有多种合理选择时**：列出选项和各自的理由，让用户选
- **发现技术方案中有歧义或遗漏时**：不要自己脑补，回去确认

不需要确认的事情就不要问。如果切片划分清晰、粒度合理、顺序明确，直接往下走。
...
- 坏的："API 返回正确数据"
- 好的："POST /api/login 传入 `{phone: '13800138000', password: '123456'}` → 返回 200，body 中包含 token 字段（非空字符串）"
...
示例：
- 通俗解释："完成后，用户在列表顶部能看到一个搜索框，输入关键词回车后列表会刷新"
...
**测试是每个任务的必经阶段，必须在验证标准中体现 TDD 检查项。**

每个任务在执行时按 TDD 循环进行（RED → GREEN → REFACTOR），测试在 RED 阶段完成。因此：

1. **禁止规划"编写单元测试"这种独立任务**——测试是任务执行的一部分，不是独立交付物
2. **必须在验证标准中体现 TDD 要求**——每个任务的验证标准第一条应该是 `TDD 测试通过：[描述测试覆盖范围]`
3. **必须在生成的 tasks.md 中包含 TDD 流程声明**——参照模板中的 `⚠️ TDD 开发流程` 章节，确保后续执行者知道必须遵循 TDD

**为什么必须显式声明 TDD？**

验证标准写得好是 TDD 能跑起来的前提，但"验证标准能作为测试依据"不等于"执行者会先写测试"。如果 tasks.md 中没有显式的 TDD 声明，执行者很容易把验证标准理解为"功能验收标准"而非"TDD 测试标准"，导致先写实现后补测试甚至不补测试。

**生成文档时的检查清单**：
- [ ] tasks.md 包含 `⚠️ TDD 开发流程` 章节
- [ ] 每个任务的验证标准第一条是 `TDD 测试通过`
- [ ] 验证计划中每个任务的检查项包含"TDD 测试通过"
...
确认完成后：
1. 读取 `assets/task-planning-template.md` 作为生成基准
2. 填充内容，生成最终文档
3. 保存到 `specs/features/<feature-name>/tasks.md`
