...
**V6 解决**：超过 300 行 warn 提醒，超过 500 行 error 必须拆分。测试文件豁免行数限制。

**自动检查**：ESLint `max-lines` 规则（warn 级别）和 `code-quality-check.js`（500 行 error）会自动检查。
...
### 1.10 禁止弱默认密码（No Weak Default Password）

**V6 问题**：[fleet_drivers.py](../../apps/server/app/api/v1/fleet_drivers.py) 创建驾驶员时，用手机号当默认密码：`password=hash_password(data.phone)`

**后果**：
- 手机号是半公开信息，攻击者可以轻易猜测新创建司机的密码
- 在司机首次登录前，账号处于高风险状态
- 同类问题也出现在 [seed.py](../../apps/server/seed.py) 中硬编码 `admin123`/`driver123`

**V6 解决**：
```python
# ❌ 禁止：手机号、工号、固定字符串作默认密码
password=hash_password(data.phone)
password=hash_password("driver123")

# ✅ 使用 secrets.token_urlsafe() 生成随机密码
import secrets
password=hash_password(secrets.token_urlsafe(12))
```

**规则**：
- 任何创建用户的代码路径，禁止使用可预测的值作初始密码
- 包括但不限于：手机号、用户名、工号、固定字符串、递增数字
- 必须使用 `secrets.token_urlsafe()` 或其他 CSPRNG 生成随机密码
...
- [ ] 确认不使用 `any` 类型
- [ ] 确认不使用 `console.log`
- [ ] 确认文件大小符合限制（>300 行 warn，>500 行 error）
- [ ] 确认函数长度符合限制（>50 行 warn，>80 行 error）
...
| 文件超过 500 行 | 必须拆分，重新设计结构 |
| 函数超过 80 行 | 必须重构，提取子函数 |
| 业务逻辑错误 | 立即修复，补充测试用例 |
